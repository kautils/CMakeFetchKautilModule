...
macro(CMakeFetchKautilModule prfx)
    ...
    if((NOT ${${m}_force_option} STREQUAL "") OR (NOT DEFINED CACHE{${__build_cache_var}}))
        if(EXISTS ${${prfx}}/CMakeLists.txt)
            file(MAKE_DIRECTORY ${${prfx}}/build)
            CMakeExecuteCommand(execgit ASSERT ${${m}_verbose_option} DIR ${${prfx}} COMMAND cmake -S . -B ${${prfx}}/build -D${m}_dest='${${m}_dest}' ${${m}_cmake_configure})
            if(DEFINED ${m}_cmake_build)
                CMakeExecuteCommand(execgit ASSERT ${${m}_verbose_option} DIR ${${prfx}} COMMAND cmake --build ${${prfx}}/build  ${${m}_cmake_build})
            else()
                CMakeExecuteCommand(execgit ASSERT ${${m}_verbose_option} DIR ${${prfx}} COMMAND cmake --build ${${prfx}}/build)
            endif()
            if(DEFINED ${m}_cmake_install)
                CMakeExecuteCommand(execgit ASSERT ${${m}_verbose_option} DIR ${${prfx}} COMMAND cmake --install ${${prfx}}/build ${${m}_cmake_install})
            else()
                CMakeExecuteCommand(execgit ASSERT ${${m}_verbose_option} DIR ${${prfx}} COMMAND cmake --install "${${prfx}}/build" --prefix "${${m}_dest}")
            endif()
